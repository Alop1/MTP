...
if(isset($_GET['id'])){
	...
	$user_profile_img_id = get_the_author_meta( 'user_profile_img', $user_id );

	if(!empty($user_profile_img_id)){
		
			$user_profile_img =  wp_get_attachment_url($user_profile_img_id);
			$user_avatar_src = $user_profile_img;
...
?>
    ...
    <div class="thumb">
        ...
        <?php
            ...
            echo '<div id="plupload-drag-drop'.$field_id.'" ><span id="plupload-browse-'.$field_id.'" class="clipart-upload button tooltip" title="'.__('Upload profile image.', 'user-profile').'"><i class="fa fa-upload"></i></span></div>';
...
<?php
	...
	?>
        ...
        <div class="profile-avatar"><?php echo apply_filters('user_profile_filter_user_avatar', $user_avatar); ?>
        </div>
