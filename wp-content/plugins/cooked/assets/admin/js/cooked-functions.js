                ...
                functionReady: function(instance,helper){
                	...
                	$('.cooked-save-default-all').on('click',function(e){
		            	...
		            	var thisButton = $(this),
		            		...
		            		recipe_editor_content = recipe_editor.getContent();
		            	...
		            	if ( confirm_save && !thisButton.hasClass('disabled') ){
			            	...
			            	var ajax__save_default_all = $.post(
			                    ...
			                    { action:'cooked_save_default', 'default_content':recipe_editor_content, 'update_all':true },
			                    function( result ) {
			                    	thisButton.removeClass("button-primary").addClass("button");
			                    	thisButton.text( cooked_js_vars.i18n_applied );
			                        //console.log( 'SUCCESS' );
			                        //console.log( result );
			                    })
		...
		if ( $_CookedNutritionFactsTab.length ){
			...
			$_CookedNutritionFactsTab.on('keyup','input',function(e){
				...
				var thisInput = $(this),
					...
					hasPercent = $('.cooked-nut-percent[data-labeltype="' + thisID + '"]').length;

				if ( $('.cooked-nut-label[data-labeltype="' + thisID + '"]').length ){
					if ( thisVal ){
						$('.cooked-nut-label[data-labeltype="' + thisID + '"]').html( thisVal );
						$('.cooked-nut-label[data-labeltype="' + thisID + '"]').parents('li').eq(0).show();
						$('.cooked-nut-label[data-labeltype="' + thisID + '"]').parents('p').eq(0).show();
						if ( hasPercent ){
							var thisPercentElem = $('.cooked-nut-percent[data-labeltype="' + thisID + '"]'),
								pvd = thisPercentElem.data('pdv');
							if ( pvd ){
								thisPercent = Math.round( ( thisVal / pvd ) * 100 );
								thisPercentElem.text( thisPercent );
							}
						}
					} else {
						$('.cooked-nut-label[data-labeltype="' + thisID + '"]').html( '___ ' );
						$('.cooked-nut-label[data-labeltype="' + thisID + '"]').parents('li').eq(0).hide();
						$('.cooked-nut-label[data-labeltype="' + thisID + '"]').parents('p').eq(0).hide();
					}
				}
