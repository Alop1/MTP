...
$c2_recipe_settings = Cooked_Recipes::get_c2_recipe_meta( $recipe_id );
if( !isset($recipe_settings['cooked_version']) && !empty($c2_recipe_settings) ):
	$recipe_review_required = true;
	$recipe_settings = Cooked_Recipes::sync_c2_recipe_settings($c2_recipe_settings,$recipe_id);
endif;

$recipe_settings['id'] = $recipe_id;
...
if ( is_singular() && !is_admin() ):
	...
	$recipe_content .=  '<div id="cooked-fsm-' . $recipe_id . '" class="cooked-fsm" data-recipe-id="' . $recipe_id . '">';
		$recipe_content .=  do_shortcode( Cooked_Recipes::fsm_content() );
		$recipe_content .=  '<div class="cooked-fsm-top">' . $recipe_settings['title'] . '<a href="#" class="cooked-close-fsm"><i class="cooked-icon cooked-icon-close"></i></a></div>';
