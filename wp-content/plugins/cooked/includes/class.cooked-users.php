...
class Cooked_Users {
	...
	public static function recipe_author_rewrite() {
		...
		if ( $browse_page_id != $front_page_id ):
			...
			if ( isset( $_cooked_settings['browse_page'] ) ):
				add_rewrite_rule('^' . $browse_page_slug . '/' . $_cooked_settings['recipe_author_permalink'] . '/([^/]*)/([^/]*)/page/([^/]*)/?', 'index.php?page_id=' . $_cooked_settings['browse_page'] . '&paged=$matches[3]&recipe_author=$matches[1]', 'top' );
				add_rewrite_rule('^' . $browse_page_slug . '/' . $_cooked_settings['recipe_author_permalink'] . '/([^/]*)/?', 'index.php?page_id=' . $_cooked_settings['browse_page'] . '&recipe_author=$matches[1]', 'top' );
	  		endif;
	...
	public static function get( $user_id ){	
		...
		if ( isset( $_user_meta['profile_photo'] ) && $_user_meta['profile_photo'] && wp_attachment_is_image( $_user_meta['profile_photo'] ) ):
			$profile_photo = wp_get_attachment_image( $_user_meta['profile_photo'], 'thumbnail' );
			$profile_photo_src = wp_get_attachment_image_src( $_user_meta['profile_photo'], 'thumbnail' );
		else :
			$profile_photo = get_avatar( $_user->user_email, 'thumbnail' );
			$profile_photo_src = get_avatar_url( $_user->user_email, 'thumbnail' );
		endif;
