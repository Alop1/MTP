...
class Cooked_Recipe_Meta {
		...
		if (!empty($recipe_settings)):
        	foreach($recipe_settings as $key => $val):
        		if (!is_array($val)):
        			...
        			if ($key === "excerpt"):
        				$recipe_excerpt = sanitize_text_field($val);
        			endif;

        			if ($key === "content"):
        				$recipe_settings[$key] = wp_kses_post( $val );
        			else:
        				$recipe_settings[$key] = sanitize_text_field($val);
        			endif;

        		else:
        			foreach($val as $subkey => $subval):
        				if (!is_array($subval)):
		        			$recipe_settings[$key][$subkey] = sanitize_text_field($subval);
		        		else:
		        			foreach($subval as $sub_subkey => $sub_subval):
		        				if (!is_array($sub_subval)):
		        					if ($sub_subkey == 'content' || $key == 'ingredients' && $sub_subkey == 'name' || $key == 'ingredients' && $sub_subkey == 'section_heading_name' || $key == 'directions' && $sub_subkey == 'section_heading_name'):
		        						$recipe_settings[$key][$subkey][$sub_subkey] = wp_kses_post( $sub_subval );
		        					else:
				        				$recipe_settings[$key][$subkey][$sub_subkey] = sanitize_text_field($sub_subval);
				        			endif;
				        		else:
				        			foreach($sub_subval as $sub_sub_subkey => $sub_sub_subval):
				        				if (!is_array($sub_sub_subval)):
						        			$recipe_settings[$key][$subkey][$sub_subkey][$sub_sub_subkey] = sanitize_text_field($sub_sub_subval);
						        		else:
						        			foreach($sub_sub_subval as $sub_sub_sub_subkey => $sub_sub_sub_subval):
						        				$recipe_settings[$key][$subkey][$sub_subkey][$sub_sub_subkey][$sub_sub_sub_subkey] = sanitize_text_field($sub_sub_sub_subval);
						        			endforeach;
						        		endif;
				        			endforeach;
				        		endif;
    ...
    public function save_recipe_meta_box( $post_id ) {
        ...
        $nonce = $_POST['cooked_recipe_custom_box_nonce'];

        // Verify that the nonce is valid.
        if ( ! wp_verify_nonce( $nonce, 'cooked_recipe_custom_box' ) )
            return $post_id;
        ...
        $recipe_settings = isset($_POST['_recipe_settings']) ? $_POST['_recipe_settings'] : false;
        $recipe_settings = self::meta_cleanup( $recipe_settings );
...
function cooked_recipe_shortcodes_content(){
	...
	?><div class="recipe-setting-block">
		...
		<div class="cooked-bm-20 cooked-block">
			<input class='cooked-shortcode-field' type='text' readonly value='[cooked-recipe id="<?php echo $post_id; ?>"]' />
		</div>
...
function cooked_render_recipe_fields( $post_id ) {
	...
	$recipe_tabs = apply_filters( 'cooked_recipe_admin_tabs', array(
		'content' => array(
			'icon' => 'desktop',
			'name' => esc_html__('Layout & Content','cooked'),
			'conditional' => false,
			'value' => false
		),
		'ingredients' => array(
	...
	if (!empty($recipe_tabs)):
		...
		foreach($recipe_tabs as $slug => $tab):
			...
			echo "<li id='cooked-recipe-tab-$slug'" . (!empty($classes) ? " class='".implode(" ",$classes)."'" : "") . ($tab['conditional'] ? " data-condition='".$tab['conditional']."'" : "") . ($tab['value'] ? " data-value='".$tab['value']."'" : ""). ">";
			echo $tab['icon'] ? "<i class='cooked-icon cooked-icon-fw cooked-icon-".$tab['icon']."'></i>&nbsp;&nbsp;" : "";
			echo $tab['name'];
			echo "</li>";
			$first_tab = false;
			...
			<?php if ( isset($recipe_settings) && !empty($recipe_settings) ): ?>
				<div class="recipe-setting-block">
					...
					<div class="cooked-bm-30 cooked-block">
						<input style="width:100%;" class="cooked-shortcode-field" type="text" readonly="" value="[cooked-recipe id=&quot;<?php echo $post_id; ?>&quot;]">
					</div>
				</div>
			<?php endif; ?>
				...
				<div class="recipe-setting-block">
					<div class="cooked-clearfix">
						...
						<div class="cooked-setting-column-13">
							...
							<select name="_recipe_settings[difficulty_level]">
								...
								<?php foreach($difficulty_levels as $level => $name):
									echo '<option value="' . $level . '"' . ( isset($recipe_settings['difficulty_level']) && $recipe_settings['difficulty_level'] == $level ? ' selected' : '' ) . '>' . esc_attr( $name ) . '</option>';
								endforeach; ?>
							</select>
						</div>
						<div class="cooked-setting-column-13">
							<h3 class="cooked-settings-title"><?php esc_html_e( 'Prep Time', 'cooked' ); ?></h3>
							<input class="cooked-time-picker" type="number" step="any" name="_recipe_settings[prep_time]" value="<?php echo ( isset($recipe_settings['prep_time']) && $recipe_settings['prep_time'] ? $recipe_settings['prep_time'] : '' ); ?>" placeholder="--">
						...
						<div class="cooked-setting-column-13">
							...
							<input class="cooked-time-picker" type="number" step="any" name="_recipe_settings[cook_time]" value="<?php echo ( isset($recipe_settings['cook_time']) && $recipe_settings['cook_time'] ? $recipe_settings['cook_time'] : '' ); ?>" placeholder="--">
							<span class="cooked-time-picker-text"><?php esc_html_e('minutes','cooked'); ?></span>
						</div>
					</div>
				</div>
		...
		<section class="cooked-recipe-tab-content" id="cooked-recipe-tab-content-ingredients">
			...
			<div id="cooked-ingredients-builder" class="cooked-sortable">
				...
				<?php if ( isset($recipe_settings['ingredients']) && !empty($recipe_settings['ingredients']) ): ?>
					...
					<?php foreach($recipe_settings['ingredients'] as $ing_key => $value): ?>
						...
						<?php if ( !isset($value['section_heading_name']) ): ?>
							...
							<div class="<?php echo $ingredient_classes; ?>">
								...
								<div class="cooked-ingredient-amount">
									<input type="text" data-ingredient-part="amount" name="_recipe_settings[ingredients][<?php echo $ing_key; ?>][amount]" value="<?php echo esc_attr( $value['amount'] ); ?>" placeholder="--">
								</div>

								<?php do_action( 'cooked_after_ingredient_amount_field', $ing_key, $value ); ?>

								<div class="cooked-ingredient-measurement">
									<select data-ingredient-part="measurement" name="_recipe_settings[ingredients][<?php echo $ing_key; ?>][measurement]">
										<option value="">--</option>
										<?php foreach($measurements as $key => $measurement):
											echo '<option value="' . $key . '"' . ( $value['measurement'] == $key ? ' selected' : '' ) . '>' . esc_attr($measurement['plural_abbr']) . '</option>';
										endforeach; ?>
									</select>
								</div>
								...
								<div class="cooked-ingredient-name">
									<input type="text" data-ingredient-part="name" name="_recipe_settings[ingredients][<?php echo $ing_key; ?>][name]" value="<?php echo esc_attr( $value['name'] ); ?>" placeholder="<?php esc_html_e('ex. Eggs, Milk, etc.','cooked'); ?> ...">
								</div>
						...
						<?php elseif ( isset($value['section_heading_name']) ): ?>
							...
							<div class="recipe-setting-block cooked-ingredient-block cooked-ingredient-heading cooked-clearfix">
								...
								<div class="cooked-heading-name">
									<input type="text" data-ingredient-part="section_heading_name" name="_recipe_settings[ingredients][<?php echo $ing_key; ?>][section_heading_name]" value="<?php echo esc_attr( $value['section_heading_name'] ); ?>" placeholder="<?php esc_html_e('Section Heading','cooked'); ?> ...">
								</div>
								<span href="#" class="cooked-delete-ingredient"><i class="cooked-icon cooked-icon-times"></i></span>
							</div>
				...
				<?php else:
					...
					$random_key = rand(100000000000,999999999999); ?>
					...
					<div class="recipe-setting-block cooked-ingredient-block cooked-clearfix">
						...
						<div class="cooked-ingredient-amount">
							<input type="text" data-ingredient-part="amount" name="_recipe_settings[ingredients][<?php echo $random_key; ?>][amount]" value="" placeholder="--">
						</div>

						<?php do_action( 'cooked_after_ingredient_amount_field', $random_key, false ); ?>

						<div class="cooked-ingredient-measurement">
							<select data-ingredient-part="measurement" name="_recipe_settings[ingredients][<?php echo $random_key; ?>][measurement]">
								<option value="">--</option>
								<?php foreach($measurements as $key => $measurement):
									echo '<option value="'.$key.'">'.esc_attr($measurement['plural_abbr']).'</option>';
								endforeach; ?>
							</select>
						</div>
						...
						<div class="cooked-ingredient-name">
							<input type="text" data-ingredient-part="name" name="_recipe_settings[ingredients][<?php echo $random_key; ?>][name]" value="" placeholder="<?php esc_html_e('ex. Eggs, Milk, etc.','cooked'); ?> ...">
						</div>
					...
					<?php foreach($recipe_settings['directions'] as $dir_key => $value): ?>
						...
						<?php if ( !isset($value['section_heading_name']) ): ?>
							...
							<?php if ( isset($value['image']) && $value['image'] ):
								...
								$image_thumb = wp_get_attachment_image( $value['image'], 'thumbnail', false, array(
									...
									'id' => 'direction-' . $dir_key . '-image-src' )
								);
							...
							<div class="recipe-setting-block cooked-direction-block cooked-clearfix">
								...
								<div class="cooked-direction-image<?php echo ( $image_thumb ? ' cooked-has-image' : '' ); ?>">
									<input data-direction-part="image" type="hidden" name="_recipe_settings[directions][<?php echo $dir_key; ?>][image]" id="direction-<?php echo $dir_key; ?>-image" value="<?php if ( isset ( $value['image'] ) ) echo esc_attr( $value['image'] ); ?>" />
									<input data-direction-part="image_button" type="button" data-id="<?php echo $dir_key; ?>" class="button direction-image-button" value="<?php echo ( $image_thumb ? esc_html__( 'Change Image', 'cooked' ) : esc_html__( 'Add Image', 'cooked' ) ); ?>" />
									<?php echo ( $image_thumb ? $image_thumb : '<img id="direction-' . $dir_key . '-image-src" data-direction-part="image_src" class="cooked-direction-img" src="" data-id="' . $dir_key . '">' ); ?>
									<div class="cooked-direction-img-placeholder"></div>
									<a href="#" data-id="<?php echo $dir_key; ?>" class="remove-image-button"><i class="cooked-icon cooked-icon-times"></i></a>
								</div>
								<div class="cooked-direction-content">
									<textarea data-direction-part="content" name="_recipe_settings[directions][<?php echo $dir_key; ?>][content]"><?php echo $value['content']; ?></textarea>
								</div>
						...
						<?php elseif ( isset($value['section_heading_name']) ): ?>
							...
							<div class="recipe-setting-block cooked-direction-block cooked-direction-heading cooked-clearfix">
								<i class="cooked-icon cooked-icon-drag"></i>
								<div class="cooked-heading-name">
									<input type="text" data-direction-part="section_heading_name" name="_recipe_settings[directions][<?php echo $dir_key; ?>][section_heading_name]" value="<?php echo esc_attr( $value['section_heading_name'] ); ?>" placeholder="<?php esc_html_e('Section Heading','cooked'); ?> ...">
								</div>
								<a href="#" class="cooked-delete-direction"><i class="cooked-icon cooked-icon-times"></i></a>
							</div>
				...
				<?php else:
					...
					<div class="recipe-setting-block cooked-direction-block cooked-clearfix">
						...
						<div class="cooked-direction-image">
							...
							<input data-direction-part="image_button" type="button" data-id="<?php echo $random_key; ?>" class="button direction-image-button" value="<?php esc_html_e( 'Add Image', 'cooked' ); ?>" />
						...
						<div class="cooked-direction-content">
							<textarea data-direction-part="content" name="_recipe_settings[directions][<?php echo $random_key; ?>][content]"></textarea>
						</div>
						<a href="#" class="cooked-delete-direction"><i class="cooked-icon cooked-icon-times"></i></a>
					</div>
				...
				<div class="cooked-clearfix">
						...
						foreach( $_nutrition_facts as $nutrition_facts ):
							foreach( $nutrition_facts as $slug => $nf ):
								...
								echo '<span class="cooked-measurement-column">';
									echo '<label for="' . $slug . '" class="cooked-nutrition-label">' . $nf['name'] . '</label>';
									echo '<input type="' . $nf['type'] . '"' . ( $nf['type'] == 'number' ? ' step="any"' : '' ) . ' placeholder="--" name="_recipe_settings[nutrition][' . $slug . ']" id="' . $slug . '" value="' . ( isset($recipe_settings['nutrition'][$slug]) ? esc_attr($recipe_settings['nutrition'][$slug]) : '' ) . '" />';
								echo '</span>';
								...
								if ( isset($nf['subs']) ):
									foreach( $nf['subs'] as $sub_slug => $sub_nf ):
										...
										if ( $nut_loops == 1 ): echo '<p class="cooked-measurement-inputs">'; endif;
										echo '<span class="cooked-measurement-column">';
											echo '<label for="' . $sub_slug . '" class="cooked-nutrition-label">' . $sub_nf['name'] . '</label>';
											echo '<input type="' . $sub_nf['type'] . '"' . ( $nf['type'] == 'number' ? ' step="any"' : '' ) . ' placeholder="--" name="_recipe_settings[nutrition][' . $sub_slug . ']" id="' . $sub_slug . '" value="' . ( isset($recipe_settings['nutrition'][$sub_slug]) ? esc_attr($recipe_settings['nutrition'][$sub_slug]) : '' ) . '" />';
										echo '</span>';
										if ( $nut_loops == 2 ): echo '</p>'; $nut_loops = 0; endif;
									endforeach;
								endif;
					...
					<div class="cooked-setting-column-12">
						<section id="cooked-nutrition-label" class="cooked-nut-label-1">
							...
							foreach( $nutrition_facts as $slug => $nf ):
								echo '<p>' . $nf['name'] . ' <strong class="cooked-nut-label" data-labeltype="' . $slug . '">___</strong></p>';
							endforeach; ?>
							...
							<ul>
								...
								foreach( $nutrition_facts as $slug => $nf ):
									if ( $slug != 'calories_fat' ):
										echo '<li><strong>' . $nf['name'] . '</strong> <strong class="cooked-nut-label" data-labeltype="' . $slug . '">___</strong>';
								...
								foreach( $nutrition_facts as $slug => $nf ):
									...
									echo '<li>';
									echo '<strong>' . $nf['name'] . '</strong> <strong class="cooked-nut-label" data-labeltype="' . $slug . '">___ </strong>' . ( isset($nf['measurement']) ? '<strong class="cooked-nut-label" data-labeltype="' . $slug . '_measurement">' . $nf['measurement'] . '</strong>' : '' );
									echo ( isset( $nf['pdv'] ) ? '<strong class="cooked-nut-right"><span class="cooked-nut-percent" data-pdv="' . $nf['pdv'] . '" data-labeltype="' . $slug . '">0</span>%</strong>' : '' );

									if ( isset($nf['subs']) ):
										foreach( $nf['subs'] as $sub_slug => $sub_nf ):
											echo '<ul><li>';
												echo '<strong>' . $sub_nf['name'] . '</strong> <strong class="cooked-nut-label" data-labeltype="' . $sub_slug . '">___ </strong>' . ( isset($sub_nf['measurement']) ? '<strong class="cooked-nut-label" data-labeltype="' . $sub_slug . '_measurement">' . $sub_nf['measurement'] . '</strong>' : '' );
												echo ( isset( $sub_nf['pdv'] ) ? '<strong class="cooked-nut-right"><span class="cooked-nut-percent" data-pdv="' . $sub_nf['pdv'] . '" data-labeltype="' . $sub_slug . '">0</span>%</strong>' : '' );
											echo '</li></ul>';
										endforeach;
									endif;
							...
							<ul class="cooked-nut-bottom cooked-clearfix">
								...
								foreach( $nutrition_facts as $slug => $nf ):
									echo '<li>';
									echo '<strong>' . $nf['name'] . ' <span class="cooked-nut-right"><span class="cooked-nut-percent cooked-nut-label" data-labeltype="' . $slug . '">0</span>%</span></strong>';
									echo '</li>';
								endforeach; ?>
							</ul>
		...
		<section class="cooked-recipe-tab-content" id="cooked-recipe-tab-content-gallery">
			...
			<div class="recipe-setting-block">
				...
				<select id="cooked_gallery_type" name="_recipe_settings[gallery][type]">
					<?php foreach( $cooked_gallery_types as $slug => $gtype ):
						echo '<option value="' . $slug . '"' . ( isset($recipe_settings['gallery']['type']) && $recipe_settings['gallery']['type'] == $slug ? ' selected' : '' ) . '>' . $gtype['title'] . '</option>';
					endforeach; ?>
				</select>
			</div>
			...
			<?php foreach( $cooked_gallery_types as $slug => $gtype ):
				if ( isset($gtype['posts']) && !empty($gtype['posts']) ): ?>
					<div class="recipe-setting-block cooked-conditional-hidden" data-condition="cooked_gallery_type" data-value="<?php echo $slug; ?>">
						<h3 class="cooked-settings-title"><?php echo $gtype['title']; ?></h3>
						<select id="cooked_gallery_type" name="_recipe_settings[gallery][<?php echo $slug; ?>]">
							<option value=""><?php esc_html_e('Choose one...','cooked'); ?></option>
							<?php foreach( $gtype['posts'] as $gid => $g ):
								echo '<option value="' . $gid . '"' . ( isset($recipe_settings['gallery'][$slug]) && $recipe_settings['gallery'][$slug] == $gid ? ' selected' : '' ) . '>' . $g . '</option>';
							endforeach; ?>
						</select>
					</div>
				<?php endif;
				...
				<div id="cooked-recipe-image-gallery" class="cooked-clearfix"><?php
					...
					if (!empty($gallery_items)):
						foreach ($gallery_items as $g_item):
							...
							echo '<div data-attachment-id="' . $g_item . '" class="cooked-recipe-gallery-item">' . $image_thumb . '<span class="cooked-gallery-item-title">' . $image_title . '</span><input type="hidden" name="_recipe_settings[gallery][items][]" value="' . $g_item . '" /><a href="#" data-attachment-id="' . $g_item . '" class="cooked-gallery-edit-button"><i class="cooked-icon cooked-icon-pencil"></i></a><a href="#" class="remove-image-button"><i class="cooked-icon cooked-icon-times"></i></a></div>';
					...
					<div class="cooked-setting-column-13">
						...
						<p class="cooked-bm-10">
							...
							<?php $available_cooked_info_vars = apply_filters( 'cooked_available_info_vars', array(
								...
								'difficulty' => esc_html__( 'Difficulty','cooked' ),
							...
							foreach( $available_cooked_info_vars as $var => $name ):
								echo '<strong>' . $var . '</strong> (' . $name . ')<br>';
							endforeach; ?>
