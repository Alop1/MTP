...
class Cooked_Shortcodes {
	...
	public function cooked_search_shortcode( $atts, $content = null ){
		...
		Cooked_Recipes::recipe_search_box( $options );
	...
	public function cooked_browse_shortcode( $sc_atts, $content = null ){
		...
		if ( isset($_cooked_settings['advanced']) && !empty($_cooked_settings['advanced']) && in_array( 'disable_public_recipes', $_cooked_settings['advanced'] ) )
			return ( current_user_can( 'administrator' ) ? wpautop( sprintf( esc_html__('Public recipes are currently disabled. You can change this at the bottom of the %s page.','cooked'), '<a href="' . trailingslashit( admin_url() ) . 'admin.php?page=cooked_settings" target="_blank">' . esc_html__( 'Settings', 'cooked' ) . '</a>' ) ) : false );
	...
	public function cooked_gallery_shortcode($atts, $content = null) {
		...
		if ( isset($recipe_settings['gallery']) && isset($recipe_settings['gallery']['type']) ):
			...
			switch( $recipe_settings['gallery']['type'] ):
				...
				case 'envira':
					return ( $recipe_settings['gallery']['envira'] ? '<div class="cooked-recipe-gallery">' . do_shortcode('[envira-gallery id="' . $recipe_settings['gallery']['envira'] . '"]') . '</div>' : '' );
				break;

				case 'soliloquy':
					return ( $recipe_settings['gallery']['soliloquy'] ? '<div class="cooked-recipe-gallery">' . do_shortcode('[soliloquy id="' . $recipe_settings['gallery']['soliloquy'] . '"]') . '</div>' : '' );
				break;

				case 'revslider':
					return ( $recipe_settings['gallery']['revslider'] ? '<div class="cooked-recipe-gallery">' . do_shortcode('[rev_slider alias="' . $recipe_settings['gallery']['revslider'] . '"]') . '</div>' : '' );
				break;
	...
	public static function cooked_info_author(){
		...
		if (in_array('author',$_cooked_settings['recipe_info_display_options'])):
			...
			echo '<span class="cooked-author"><span class="cooked-author-avatar">' . $author['profile_photo'] . '</span><strong class="cooked-meta-title">' . esc_html__('Author','cooked') . '</strong>' . ( $clickable ? '<a href="' . $permalink . '">' : '' ) . $author['name'] . ( $clickable ? '</a>' : '' ) . '</span>';
			wp_reset_postdata();
	...
	public static function cooked_info_difficulty( $recipe ){
		...
		if (in_array('difficulty_level',$_cooked_settings['recipe_info_display_options'])):
			if ( isset($recipe['difficulty_level']) && $recipe['difficulty_level'] ):
				echo '<span class="cooked-difficulty-level"><strong class="cooked-meta-title">' . esc_html__('Difficulty','cooked') . '</strong>' . Cooked_Recipes::difficulty_level( $recipe['difficulty_level'] ) . '</span>';
			endif;
		endif;
	}
		...
		if ( isset($recipe_settings['directions']) && !empty($recipe_settings['directions']) ):
			echo '<div class="cooked-recipe-directions">';
				foreach( $recipe_settings['directions'] as $dir ):
					...
					Cooked_Recipes::single_direction( $dir, $number, false, $step );
					...
					if ( isset($nf['subs']) ):
						foreach( $nf['subs'] as $sub_slug => $sub_nf ):
							...
							echo '<dl><dt>';
								...
								echo ( isset( $sub_nf['pdv'] ) && $nutrition_facts[$sub_slug] ? '<strong class="cooked-nut-right"><span class="cooked-nut-percent">' . ceil( ( esc_attr( $nutrition_facts[$sub_slug] ) / $nf['pdv'] ) * 100 ) . '</span>%</strong>' : '' );
							echo '</dt></dl>';
							endif;
						endforeach;
					endif;
